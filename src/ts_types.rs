...
fn pascal_case(
    ...
    let mut s: String = s.split("/").last().map(|s| s.split(".").into_iter().collect()).unwrap();
    let mut c = s.chars();
    match c.next() {
        None => String::new(),
        Some(f) =>
        ...
        "object" => {
            if let Some(properties) = &value.properties {
                ...
                for (k, v) in properties {
                    writeln!(result, "{}:{}", k, get_response_type_with_cb(v, cb));
                }
                result.push('}');
                match value.__ref {
                    Some(ref ref1) => {
...
pub fn generate(data: &Vec<YapiObj>) -> AnyResult<()> {
    let mut map: Arc<Mutex<HashMap<String, String>>> = Arc::new(Mutex::new(HashMap::new()));
    let mut cb = |key: String, obj_str: String| {
        let mut map = map.lock().unwrap();
        if !map.contains_key(&key) {
            map.insert(key, obj_str);
        };
        // println!("{}: {}", key, obj_str);
        ()
    };
    ...
    writeln!(response_header, "import {{ {} }} from './types';", response_deps.into_iter().collect::<Vec<String>>().join(", "))?;
    writeln!(request_header, "import {{ {} }} from './types';", request_deps.into_iter().collect::<Vec<String>>().join(", "))?;
    ...
    if !map.lock().unwrap().is_empty() {
        let mut file = OpenOptions::new()
            ...
            .create(true)
            .open("types.ts")?;
        let mut types = String::from("// generated by https://github.com/spike2044/yapi2js\n");
        for (k, v) in map.lock().unwrap().iter() {
            types.push_str(&format!("export type {} = {}\n", k, v));
        }
...
pub fn generate_request(item: &YapiItem, obj: &YapiObj, cb: &mut impl FnMut(String, String)) -> AnyResult<String> {
    let re = Regex::new(r"^\w+$").unwrap();
    let mut result = String::new();
    ...
    let mut data = match item.req_body_type {
        ...
        ReqBodyType::Json => match item.req_body_other {
            Some(ref body) => {
                let v: ResponseValueType = serde_json::from_str(body)?;
                get_response_type_with_cb(&v, cb).to_string()
            }
            None => String::from(""),
        },
    };
